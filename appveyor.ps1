...
switch ($env:RUN_ITS)
{
	...
	"true"
	{
		...
		try
		{
			mvn install "--batch-mode" "-Dsonar.stylecop.msBuildPath=${env:sonar.stylecop.msBuildPath}" "-Dsonar.stylecop.styleCopDllPath=${env:sonar.stylecop.styleCopDllPath}" "-DstylecopVersion=DEV" "-DcsharpVersion=4.1" "-Dsonar.runtimeVersion=$env:SQ_VERSION" "-Dmaven.test.redirectTestOutputToFile=false" "-Dsonar.jdbc.dialect=embedded" "-Dorchestrator.updateCenterUrl=http://update.sonarsource.org/update-center-dev.properties" "-Dmaven.localRepository=$env:USERPROFILE\.m2\repository"
			CheckLastExitCode
		}
		finally
		{
